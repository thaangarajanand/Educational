...
import { Shield, Sparkles, Key, UserCheck, AlertCircle, HelpCircle, ChevronDown, ChevronUp } from 'lucide-react';
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      if (!isSupabaseConfigured()) {
        throw new Error(
          'Google OAuth is not yet configured with Supabase credentials. Please set VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY in your frontend environment file.'
        );
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-4 text-white">
      ...
      <div className="relative w-full max-w-md bg-slate-900/90 border border-slate-800 backdrop-blur-xl p-8 rounded-2xl shadow-2xl space-y-6">
        ...
        {!isConfigured && (
          <div className="bg-amber-950/40 border border-amber-800/50 rounded-xl p-3 text-xs text-amber-200/90 space-y-1">
            <div className="flex items-center gap-1.5 font-medium text-amber-300">
              <HelpCircle className="w-4 h-4" />
              <span>Supabase Setup Required</span>
            </div>
            <p className="text-[11px] leading-relaxed text-amber-300/80">
              To enable live Google OAuth redirect, add <code className="bg-amber-900/60 px-1 py-0.5 rounded text-amber-200">VITE_SUPABASE_URL</code> and <code className="bg-amber-900/60 px-1 py-0.5 rounded text-amber-200">VITE_SUPABASE_ANON_KEY</code> to your environment file.
            </p>
          </div>
        )}
